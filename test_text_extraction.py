...
def test_format_searchable_text():
    ...
    # Test with simple text (no YAML indentation - text goes in <details> block)
    text = "Hello world\nThis is a test"
    ...
    # Test with multiple blank lines (should collapse consecutive blanks)
    text = "Line 1\n\n\nLine 2\n\nLine 3"
    result = sync_manager._format_searchable_text(text)
    lines = result.split('\n')
...
def test_markdown_generation():
    """Test that markdown files are generated with hidden searchable content."""
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        # Verify _generate_markdown_content exists
        assert hasattr(sync_manager, '_generate_markdown_content'), "Missing _generate_markdown_content method"
        print("  ✓ _generate_markdown_content method exists")
...
def main():
    ...
    try:
        ...
        print("  - Text formatting for hidden searchable blocks implemented")
        print("  - Metadata storage supports text content (backward compatible)")
        print("  - Markdown generation includes hidden searchable content")
        print()
